...
export default function ItineraryPOI({ itinerary_id, itinerary_index }: Props) {
    ...
    const handleTravelMethodChange = async (itinerary_poi_id: number) => async (travel_method: string) => {
        const { error } = await supabase
            .from("itinerary_poi")
            .update({ travel_method })
            .eq("itinerary_poi_id", itinerary_poi_id);

        if (error) {
            console.error("Error updating travel method", error);
        }
    }
